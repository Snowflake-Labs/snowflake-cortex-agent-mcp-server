...
snow sql --stdin <<EOF
    ...
    GRANT ROLE $SNOWFLAKE_ROLE TO USER $SA_USER;
EOF
...
create or replace network rule $SNOWFLAKE_DATABASE.networks.pat_local_access_rule
  mode = ingress
  type = ipv4
...
allowed_network_rule_list = ('"$SNOWFLAKE_DATABASE".networks.pat_local_access_rule')
...
snow sql --stdin <<EOF
create or replace authentication policy $SNOWFLAKE_DATABASE.policies."${USER}_mcp_auth_policy"
 ...
 alter user $SA_USER set AUTHENTICATION POLICY $SNOWFLAKE_DATABASE.policies."${USER}_mcp_auth_policy";
EOF
...
if [ -z "$EXISTING_PAT" ]; then
  ...
  SNOWFLAKE_SA_PASSWORD=$(snow sql \
    --query "ALTER USER IF EXISTS $SA_USER ADD PAT mcp_demo ROLE_RESTRICTION = $SNOWFLAKE_ROLE" \
